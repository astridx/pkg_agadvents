...
class AdministratorService
{
	 ...
	 * @throws  Exception
	 */
	public function association($agadventid)
	{
		// Defaults
		$html = '';

		// Get the associations
		if ($associations = Associations::getAssociations('com_agadvents', '#__agadvents_details', 'com_agadvents.item', $agadventid, 'id', null)) {
			foreach ($associations as $tag => $associated) {
				$associations[$tag] = (int) $associated->id;
			}

			// Get the associated agadvent items
			$db = Factory::getDbo();
			$query = $db->getQuery(true)
				->select('c.id, c.name as title')
				->select('l.sef as lang_sef, lang_code')
				->from('#__agadvents_details as c')
				->select('cat.title as category_title')
				->join('LEFT', '#__categories as cat ON cat.id=c.catid')
				->where('c.id IN (' . implode(',', array_values($associations)) . ')')
				->where('c.id != ' . $agadventid)
			...
			if ($items) {
				foreach ($items as &$item) {
					...
					$url = Route::_('index.php?option=com_agadvents&task=agadvent.edit&id=' . (int) $item->id);
	 ...
	 *
	 * @return  string	The anchor tag to toggle featured/unfeatured agadvents.
	 *
	 * @since   __BUMP_VERSION__
	 */
	public function featured($value, $i, $canChange = true)
	{
		...
		$states = [
			0 => ['unfeatured', 'agadvents.featured', 'COM_CONTACT_UNFEATURED', 'JGLOBAL_ITEM_FEATURE'],
			1 => ['featured', 'agadvents.unfeatured', 'JFEATURED', 'JGLOBAL_ITEM_UNFEATURE'],
		];
