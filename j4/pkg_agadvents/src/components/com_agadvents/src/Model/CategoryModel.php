...
/**
 * Single item model for a agadvent
 *
 * @package     Joomla.Site
 * @subpackage  com_agadvents
...
class CategoryModel extends ListModel
{
	...
	/**
	 * Array of agadvents in the category
	 *
	 * @var    \stdClass[]
	 */
	...
	public function getItems()
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++) {
			...
			if ($this->getState('load_tags', true)) {
				...
				$this->tags->getItemTags('com_agadvents.agadvent', $item->id);
			}
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$numberOfAgadventsToDisplay = $mergedParams->get('agadvents_display_num');

		if ($format === 'feed') {
			$limit = $app->get('feed_limit');
		} else if (isset($numberOfAgadventsToDisplay)) {
			$limit = $numberOfAgadventsToDisplay;
		} else {
		...
		$search = $app->getUserStateFromRequest('com_agadvents.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
