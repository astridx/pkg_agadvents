 ...
 * @since  3.3
 */
class AgadventsRouter extends JComponentRouterBase
{
	/**
	 * Build the route for the com_agadvents component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($query['view']))
		{
			...
			if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_agadvents')
			{
				$segments[] = $query['view'];
			}
		...
		if (isset($view) and ($view == 'category' or $view == 'agadvent' or $view == 'agadvent_no'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				...
				$menuCatid = $mId;
				$categories = JCategories::getInstance('Agadvents');
	...
	public function parse(&$segments)
	{
		...
		// Standard routing for agadvents.
		if (!isset($item))
		{
		...
		$category = JCategories::getInstance('Agadvents')->get($id);
 ...
 */
function AgadventsBuildRoute(&$query)
{
	$router = new AgadventsRouter;
 ...
 * @deprecated  4.0  Use Class based routers instead
 */
function AgadventsParseRoute($segments)
{
	$router = new AgadventsRouter;
