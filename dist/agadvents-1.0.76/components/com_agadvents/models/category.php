...
/**
 * Agadvents Component Agadvent Model
 *
 * @since  1.5
 */
class AgadventsModelCategory extends JModelList
{
	/**
	 * Category items data
	 *
	...
	/**
	 * The list of other agadvent categories.
	 *
	 * @var  array
	 */
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->tags->getItemTags('com_agadvents.agadvent', $item->id);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select($this->getState('list.select', 'a.*'))
			->from($db->quoteName('#__agadvents') . ' AS a')
		...
		// Filter on the begin and end date.
		if ($this->getState('filter.begin_ende'))
		{
			$query->where('(a.begin <= ' . $nowDate . ' AND a.ende >= ' . $nowDate . ')');
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$params = JComponentHelper::getParams('com_agadvents');
		...
		if ((!$user->authorise('core.edit.state', 'com_agadvents')) && (!$user->authorise('core.edit', 'com_agadvents')))
		{
	...
	public function getCategory()
	{
		if (!is_object($this->_item))
		{
			...
			$categories = JCategories::getInstance('Agadvents', $options);
