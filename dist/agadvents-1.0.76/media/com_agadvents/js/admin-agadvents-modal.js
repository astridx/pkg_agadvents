...
(function() {
	 ...
	 * View element calls jSelectAgadvent when an agadvent is clicked
	 * jSelectAgadvent creates the link tag, sends it to the editor,
	 * and closes the select frame.
	 **/
	window.jSelectAgadvent = function (id, title, catid, object, link, lang) {
		var hreflang = '', editor, tag;

		if (!Joomla.getOptions('xtd-agadvents')) {
			// Something went wrong!
			window.parent.jModalClose();
			return false;
		}

		editor = Joomla.getOptions('xtd-agadvents').editor;
	...
	document.addEventListener('DOMContentLoaded', function(){
		...
		for(var i = 0, l = elements.length; l>i; i++) {
			...
			elements[i].addEventListener('click', function (event) {
				...
				if (functionName === 'jSelectAgadvent') {
