 ...
 * @since  1.5
 */
class AgadventsControllerAgadvent extends JControllerForm
{
	/**
	...
	public function save($key = null, $urlVar = 'w_id')
	{
		...
		$app->setUserState('com_agadvents.edit.agadvent.data', $data);
		...
		if ($result)
		{
			...
			$app->setUserState('com_agadvents.edit.agadvent.data', null);
			$this->setRedirect($this->getReturnPage());
		}
	...
	/**
	 * Go to a agadvent and count the hits
	 *
	 * @return  void
	 *
	 * @since   1.6
	...
	public function go()
	{
		...
		$modelLink = $this->getModel('Agadvent', '', array('ignore_request' => true));
		...
		if (empty($link))
		{
			return JError::raiseWarning(404, JText::_('COM_AGADVENTS_ERROR_AGADVENT_NOT_FOUND'));
		}
		...
		$modelCat = $this->getModel('Category', 'AgadventsModel', array('ignore_request' => true));
		...
		if ($tmpl == 'component')
		{
			$link	= JRoute::_('index.php?option=com_agadvents&tmpl=component&view=agadvent&id=' . $id);
		}
		else
		{
			$link	= JRoute::_('index.php?option=com_agadvents&view=agadvent&id=' . $id);
		}
		...
		return JError::raiseWarning(404, JText::_('COM_AGADVENTS_ERROR_AGADVENT_URL_INVALID'));
	}
}
