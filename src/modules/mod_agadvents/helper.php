...
require_once JPATH_SITE . '/components/com_agadvents/helpers/category.php';

JModelLegacy::addIncludePath(JPATH_SITE . '/components/com_agadvents/models', 'AgadventsModel');

/**
 * Helper for mod_agadvents
 *
 * @since  1.5
 */
class ModAgadventsHelper
{
	/**
	 ...
	 *
	 * @return  mixed   Null if no agadvents based on input parameters else an array containing all the agadvents.
	 *
	 * @since   1.5
	 **/
	public static function getCategory(&$params)
	{
		...
		$model = JModelLegacy::getInstance('Category', 'AgadventsModel', array('ignore_request' => true));
		...
		// Access filter
		$access = !JComponentHelper::getParams('com_agadvents')->get('show_noauth');
		...
		if ($items)
		{
			foreach ($items as $item)
			{
				if ($item->params->get('count_clicks', $params->get('count_clicks')) == 1)
				{
					...
					$item->link_notactive	= JRoute::_('index.php?option=com_agadvents&view=agadvent_no&id=' . $item->slug);
					$item->link_popup	= JRoute::_('index.php?tmpl=component&option=com_agadvents&task=agadvent.go&view=agadvent&id=' . $item->slug);
					$item->link_notactive_popup	= JRoute::_('index.php?tmpl=component&option=com_agadvents&view=agadvent_no&id=' . $item->slug);
				}
				else
				{
					...
					$item->link_notactive_popup	= JRoute::_('index.php?option=com_agadvents&tmpl=component&view=agadvent_no&id=' . $item->slug);
				}
			}
