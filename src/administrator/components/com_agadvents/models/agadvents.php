...
/**
 * Methods supporting a list of agadvent records.
 *
 * @since  1.6
 */
class AgadventsModelAgadvents extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.title, a.cords, a.cords, a.number, a.alias, a.checked_out, a.checked_out_time, a.catid, a.created, a.created_by, ' .
				'a.hits, a.state, a.access, a.ordering, a.language, a.publish_up, a.publish_down'
			)
		);
		$query->from($db->quoteName('#__agadvents', 'a'));
		...
		if ($assoc)
		{
			$query->select('COUNT(asso2.id)>1 AS association')
				->join('LEFT', $db->quoteName('#__associations', 'asso') . ' ON asso.id = a.id AND asso.context = ' . $db->quote('com_agadvents.item'))
